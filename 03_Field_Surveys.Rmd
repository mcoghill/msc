...
nat_richness_norm_assum <- lapply(inv_richness_assum, "[[", "shapiro") %>% 
  do.call(rbind, .) %>% rownames_to_column("sp_richness") %>% 
  dplyr::mutate(sp_richness = gsub("\\..*", "", sp_richness))
nat_richness_var_eq_assum <- lapply(inv_richness_assum, "[[", "levene") %>% 
  do.call(rbind, .) %>% rownames_to_column("sp_richness")
...
bg_analysis <- litter %>% wilcox_test(`Bare Ground` ~ Site) %>% add_significance()
bg_effects <- litter %>% wilcox_effsize(`Bare Ground` ~ Site)
...
temp_analysis <- temp_data_df %>% wilcox_test(temp ~ Type) %>% 
...
temp_smooth_analysis <- smooth_data %>% wilcox_test(y ~ Type) %>% 
