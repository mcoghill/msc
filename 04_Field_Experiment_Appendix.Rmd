...
one_way <- lapply(seq_along(vars), function(x) {
  ...
  norm <- group_by(y, Treatment) %>% 
    shapiro_test(val) %>% 
    dplyr::mutate(normal = p > 0.05, method = "raw")
  
  # If not normal, log transform
  if(!all(norm$normal) && nrow(norm[norm$normal, ]) < 5) {
    z <- dplyr::mutate(y, val = log10(val + abs(min(val)) + 1))
    norm <- group_by(z, Treatment) %>% 
      shapiro_test(val) %>% 
      dplyr::mutate(normal = p > 0.05, method = "log")
    
    # If still not normal, square root transform
    if(!all(norm$normal) && nrow(norm[norm$normal, ]) < 5) {
      z <- dplyr::mutate(y, val = sqrt(val + abs(min(val))))
      norm <- group_by(z, Treatment) %>% 
        shapiro_test(val) %>% 
        dplyr::mutate(normal = p > 0.05, method = "sqrt")
      ...
      if(!all(norm$normal) && nrow(norm[norm$normal, ]) < 5) {
        ...
        norm <- group_by(z, Treatment) %>% 
          shapiro_test(val) %>% 
          dplyr::mutate(normal = p > 0.05, method = "sqr")
      }
    }
  } else z <- y
