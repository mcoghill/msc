...
The below chunk takes a very long time to process (so far ~16 hours total) so prepare to leave this for a while and let it run.
...
bc_all_sp_by_subzone <- sapply(unique(bc_all_sp_invaded_subzone$MAP_LABEL), function(x) {
  dplyr::filter(bc_all_sp_invaded_subzone, MAP_LABEL == x) %>% 
    group_by(SUBZONE) %>% 
    summarise(n = n(), do_union = TRUE, .groups = "drop") %>% 
    dplyr::mutate(inv_area = units::set_units(st_area(.), km^2),
                  inv_ratio = units::drop_units(inv_area / bgc_area * 100)) %>% 
...
bc_all_sp_by_zone <- lapply(bc_all_sp_invaded_subzone, function(x) {
...
bc_all_sp_tot_area <- lapply(bc_all_sp_by_subzone, function(x) {
...
bc_extra_data <- tribble(
  ~stat, ~value, 
  "total_inv_area_km2", units::drop_units(sum(bc_total_invaded_subzone_ng$total_inv_area)),
  "total_inv_ratio", units::drop_units(sum(bc_total_invaded_subzone_ng$total_inv_area) / bc_bgc_area * 100),
...
st_write(bc_total_invaded_subzone, file.path(bc_maps_dir, "bc_total_invaded_by_subzone.gpkg"), 
         delete_layer = TRUE, quiet = TRUE)
