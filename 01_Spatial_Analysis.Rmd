...
dir_out <- file.path("./01_Spatial_Data")
...
st_write(ldb_aoi, file.path(dir_out, "ldb.gpkg"), layer = "AOI",
...
st_write(ldb_bec, file.path(dir_out, "ldb.gpkg"), layer = "BEC",
         delete_layer = TRUE, quiet = TRUE)
...
ldb_sk_invaded_perc <- ldb_sk_area / ldb_area * 100 # ~13.38%
...
ldb_sk_by_zone <- dplyr::group_by(ldb_sk_by_subzone, ZONE) %>% 
  dplyr::summarise(area = sum(area), bgc_area = sum(bgc_area), .groups = "drop") %>% 
...
# Write outputs, save in single file for clarity
st_write(ldb_iapp, file.path(dir_out, "ldb.gpkg"), layer = "IAPP_All",
         delete_layer = TRUE, quiet = TRUE)
st_write(ldb_sk, file.path(dir_out, "ldb.gpkg"), layer = "IAPP_SK",
         delete_layer = TRUE, quiet = TRUE)
st_write(ldb_sk_by_subzone, file.path(dir_out, "ldb.gpkg"), layer = "SK_by_subzone",
         delete_layer = TRUE, quiet = TRUE)
st_write(ldb_sk_by_zone, file.path(dir_out, "ldb.gpkg"), layer = "SK_by_zone",
         delete_layer = TRUE, quiet = TRUE)
write.csv(ldb_stats, file.path(dir_out, "ldb_stats.csv"), row.names = FALSE)
...
bc_sk <- dplyr::filter(iapp_full, grepl("SK", MAP_LABEL)) %>% 
  st_union()
...
bc_sk_by_subzone <- sf::st_set_agr(bgc_subzones, "constant") %>% 
  sf::st_intersection(bc_sk) %>% 
  dplyr::mutate(area = conv_unit(units::drop_units(st_area(.)), "m2", "hectare"), 
                ratio = area / bgc_area * 100) %>% 
  dplyr::arrange(desc(area))

# st_read(file.path(bc_out, "BC_SK_by_subzone.gpkg"), quiet = TRUE)
...
bc_out <- file.path(dir_out, "bc")
dir.create(bc_out, showWarnings = FALSE)
st_write(bc_iapp, file.path(bc_out, "BC_IAPP_All.gpkg"), 
         delete_layer = TRUE, quiet = TRUE)
st_write(bc_sk, file.path(bc_out, "BC_IAPP_SK.gpkg"), 
         delete_layer = TRUE, quiet = TRUE)
st_write(bc_bgc_subzones, file.path(bc_out, "BC_BEC.gpkg"), 
         delete_layer = TRUE, quiet = TRUE)
st_write(bc_sk_by_subzone, file.path(bc_out, "BC_SK_by_subzone.gpkg"), 
         delete_layer = TRUE, quiet = TRUE)
write.csv(bc_sk_by_zone, file.path(bc_out, "BC_SK_by_zone.csv"), row.names = FALSE)
...
bc_invasive_plants <- st_drop_geometry(iapp_full) %>% 
...
n_splits <- max(nchar(str_replace_all(bc_invasive_plants$MAP_LABEL, "[^[:alnum:]]", "")), na.rm = TRUE)
all_plants_wide <- bc_invasive_plants %>% 
  tidyr::separate(MAP_LABEL, into = paste0("sp", 1:n_splits), sep = " ", extra = "drop")

all_plants_string <- foreach(i = 1:ncol(all_plants_wide), .combine = rbind) %do% {
  unique(all_plants_wide[, i]) %>% dplyr::rename(column = 1)
} %>% grep(pattern = "[^[:alnum:]]", x = unique(.$column), value = TRUE, invert = TRUE) %>% 
  na.omit()

# There are 185 invasive species recorded in BC. Which ones take up the largest
# amount of physical space in BC?
bc_invasive_plant_areas <- foreach(i = 1:length(all_plants_string), .combine = rbind) %do% {
  cat(paste0("Working on ", all_plants_string[i], " [", i, " of ", length(all_plants_string), "]\n"))
  dplyr::filter(iapp_full, grepl(all_plants_string[i], MAP_LABEL)) %>% 
    st_union() %>% 
    st_as_sf() %>% 
    dplyr::mutate(SPECIES = all_plants_string[i], 
                  area = conv_unit(units::drop_units(st_area(.)), "m2", "hectare"))
} %>% dplyr::rename(geometry = x) %>% 
  dplyr::select(SPECIES, area) %>% 
  dplyr::arrange(desc(area))

# st_read(file.path(bc_out, "BC_IAPP_by_plant.gpkg"), quiet = TRUE)
...
bc_stats <- tribble(
  ~stat, ~value, 
  "n_invasive_species", length(all_plants_string), 
...
st_write(bc_invasive_plant_areas, file.path(bc_out, "BC_IAPP_by_plant.gpkg"), 
         delete_layer = TRUE, quiet = TRUE)
write.csv(bc_bgc_zone_data, file.path(bc_out, "BC_BEC_zone_data.csv"), row.names = FALSE)
write.csv(bc_stats, file.path(bc_out, "BC_stats.csv"), row.names = FALSE)
...
# Write output
st_write(ldb_rra, file.path(dir_out, "ldb.gpkg"), layer = "RRAs",
         delete_layer = TRUE, quiet = TRUE)
