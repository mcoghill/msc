...
st_write(bc_invasive_plant_areas, file.path(bc_out, "BC_IAPP_by_plant.gpkg"), 
         delete_layer = TRUE, quiet = TRUE)
write.csv(bc_inv_plant_areas_no_geom, file.path(bc_out, "BC_IAPP_by_plant.csv"), row.names = FALSE)
...
unzip(file.path(dir_out, "field", "Matt.kmz"), exdir = file.path(dir_out, "field"), overwrite = TRUE)
layers <- st_layers(file.path(dir_out, "field", "doc.kml"))$name
field_data <- lapply(layers, function (x)
  st_read(file.path(dir_out, "field", "doc.kml"), layer = x, quiet = TRUE))
...
aoi_exp <- area_of_invasion %>% 
  ...
  dplyr::mutate(within = units::drop_units(st_area(aoi_within) / area)) %>% 
  dplyr::mutate(beyond = 1 - within)
