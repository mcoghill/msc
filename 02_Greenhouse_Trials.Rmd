...
art_custom <- function(x, dv) {
  ...
  sapply(unique(x$Species), function(i) {
    ...
    df <- x %>% 
      ...
      dplyr::mutate(Soil_Type = factor(Soil_Type, levels = c("Invaded", "Pristine")),
                    Soil_Treatment = factor(Soil_Treatment, levels = c("Con", "Ash", "AC"))) %>% 
      as.data.frame() # This is important for ART to run
    
    # Perform aligned rank transformation (ART)
    m <- art(var ~ Soil_Type * Soil_Treatment, data = df)
    anova <- anova(m, response = "art") %>% broom::tidy()
    
    # Note: As of Feb. 2021, there is some new syntax invovled in the ARTool
    # package that might be better to use. Will just comment it out for now
    # since thesis is done, but revisit if necessary. This is the same as running
    # emmeans::contrasts(), so it only produces t.values, not F values:
    main_type <- art.con(m, "Soil_Treatment", method = "eff", adjust = "bonferroni")
    main_trt <- art.con(m, "Soil_Type", method = "eff", adjust = "bonferroni")
