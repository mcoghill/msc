...
library(tidyverse)
library(car)
library(ggplot2) # Does this load with tidyverse?
library(ggpubr)
library(magick)
library(agricolae)
library(extrafont)
...
biomass <- read.csv(file.path(data_dir, "Biomass.csv")) %>% 
...
interactions <- ggarrange(plotlist = interactions, common.legend = TRUE, legend = "bottom")
...
biomass_plot <- ggarrange(plotlist = biomass_plot, common.legend = TRUE, legend = "bottom") +
  ggsave(file.path(data_dir, "bmass.png"), width = 6, height = 3, dpi = 300)
...
final <- biomass_plot + 
  ...
  annotation_raster(sk_img, xmin = 0.64, 
                    ...
                    ymin = 0.6,
                    ymax = 0.85) + 
  ggsave(file.path(data_dir, "bmass_with_images.png"), width = 6, height = 3, dpi = 300)
...
analyzer_plots <- sapply(elements, function(x) {
  ...
  p <- ggplot(df_summary, aes(Soil_Type, mean, fill = Soil_Treatment)) + 
    ...
    scale_fill_manual(
      ...
      name = "Soil Amendment") +
    scale_y_continuous(expand = c(0, 0), limits = c(0, max(ceiling(df$data))))+
  ...
  p2 <- ggplot(df, aes(Soil_Type, data, fill = Soil_Treatment)) +
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(min(floor(df$data)), max(ceiling(df$data))))
}, simplify = FALSE, USE.NAMES = TRUE)

final <- ggarrange(plotlist = analyzer_plots, common.legend = TRUE, legend = "bottom")
