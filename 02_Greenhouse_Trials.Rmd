...
biomass_plot <- sapply(unique(biomass_summary$Species), function(x) {
  ...
  df_summary <- HSD.test(aov(sqrt_mass ~ trt, data = df), trt = "trt", 
                         group = TRUE, 
                         alpha = 0.05 / (length(unique(df$Soil_Type)) / 2)) %>% 
    .[c("means", "groups")] %>% 
  ...
  p <- ggplot(df_summary, aes(Soil_Treatment, sqrt_mass, fill = Soil_Type)) +
    geom_bar(
      stat = "identity",
      position = "dodge",
      colour = "black",
...
art_custom <- function(x, dv) {
  ...
  sapply(unique(x$Species), function(i) {
    ...
    m_type <- artlm(m, "Soil_Type")
    m_trt <- artlm(m, "Soil_Treatment")
    
    aov_type <- m_type$model %>% 
      group_by(Soil_Treatment) %>% 
      anova_test(.y ~ Soil_Type, error = m_type)
    aov_trt <- m_trt$model %>% 
      group_by(Soil_Type) %>% 
      anova_test(.y ~ Soil_Treatment, error = m_trt)
    ...
    pwc_type <- if(all(aov_type$p < 0.05)) {
      ...
      m_type$model %>% 
        group_by(Soil_Type) %>% 
        pairwise_t_test(.y ~ Soil_Treatment, p.adjust.method = "bonferroni") %>% 
        add_xy_position(x = "Soil_Type")
    }
    
    pwc_treatment <- if(all(aov_trt$p < 0.05)) {
      m_trt$model %>% 
        group_by(Soil_Treatment) %>% 
      ...
      m_trt$model %>% 
        group_by(Soil_Treatment) %>% 
        pairwise_t_test(.y ~ Soil_Type, p.adjust.method = "bonferroni") %>% 
        add_xy_position(x = "Soil_Treatment")
    }
    
    df <- dplyr::select(df, Soil_Type, Soil_Treatment, Species, var) %>% 
      dplyr::mutate(soil_type_aligned_ranks = m_type$model$.y,
                    soil_trt_aligned_ranks = m_trt$model$.y)
    ...
    return(list(
      data = df, model = m, two_way = anova, one_way_type = aov_type,
      one_way_treatment = aov_trt, pwc_type = pwc_type, 
      pwc_treatment = pwc_treatment, bxp_type = bxp_type, bxp_trt = bxp_treatment))
  }, simplify = FALSE, USE.NAMES = TRUE) 
}

death_art <- art_custom(surv_trans, "deaths")
death_out <- sapply(names(death_art$RF)[!names(death_art$RF) %in% c(
  "model", "bxp_type", "bxp_trt")], function(x) {
...
lapply(c("death", "germ_surv", "trial_surv"), function(x) {
  ...
  write.csv(y$one_way_type, file.path(
    results_dir, paste0(id, "d_", x, "_one_way_soil_type.csv")), row.names = FALSE)
  write.csv(y$one_way_treatment, file.path(
...
surv_plots <- sapply(plots, function(x) {
  ...
  iter <- sapply(unique(df$Species), function(y) {
    ...
    df2_summary <- HSD.test(aov(data ~ trt, data = df2), trt = "trt", 
                            group = TRUE, 
                            alpha = 0.05 / length(unique(df2$Soil_Type))) %>% 
      .[c("means", "groups")] %>% 
      lapply(rownames_to_column, "trt") %>% 
      Reduce(f = function(x, y) merge(x, y), x = .) %>% 
    ...
    p <- ggplot(df2_summary, aes(Soil_Type, data, fill = Soil_Treatment)) + 
      ...
      ylab(case_when(
        ...
        x == "deaths" ~ "Death Rate",
        x == "trial_surv" ~ "Trial Survival Rate", 
        x == "germ_surv" ~ "Germination Survival Rate",
        TRUE ~ "error")) +
...
# Rearrange
surv_plots <- unlist(surv_plots[c(
  "germ_rat", "deaths", "germ_surv", "trial_surv")], recursive = FALSE)
