...
anova_analysis <- function(x, dv) {
  sapply(unique(x$Species), function(i) {
    df <- dplyr::filter(x, Species == i) %>% 
      ...
      droplevels() %>% 
      dplyr::mutate(Soil_Type = factor(Soil_Type, levels = c("Invaded", "Pristine")),
                    Soil_Treatment = factor(Soil_Treatment, levels = c("AC", "Ash", "Con"))) %>% 
      as.data.frame()
    fct <- max(df$var)
    ...
    pwc_type <- group_by(df, Soil_Type) %>% 
      ...
      add_xy_position(x = "Soil_Type", fun = "mean_sd") %>% 
      dplyr::mutate(y.position = y.position + (0.05 * fct), 
                    term = "Soil_Treatment") %>% 
      dplyr::select(-any_of(c("groups", "n1", "n2", "p.signif", "df", "statistic"))) %>% 
      as.data.frame()
    ...
    pwc_treatment <- group_by(df, Soil_Treatment) %>% 
      ...
      add_xy_position(x = "Soil_Treatment", fun = "mean_sd") %>% 
      dplyr::mutate(y.position = y.position + (0.05 * fct), term = "Soil_Type") %>% 
      dplyr::select(-any_of(c("groups", "n1", "n2", "p.signif", "df", "statistic"))) %>% 
      as.data.frame()
...
art_custom <- function(x, dv) {
  ...
  sapply(unique(x$Species), function(i) {
    ...
    # These are good
    main_type <- contrast(emmeans(artlm(m, "Soil_Treatment"), ~ Soil_Treatment), 
                         adjust = "bonferroni") %>% data.frame()
    main_trt <- contrast(emmeans(artlm(m, "Soil_Type"), ~ Soil_Type), 
                         adjust = "bonferroni") %>% data.frame()
    
    # PWC's are good!
    pwc_type <- contrast(emmeans(artlm(m, "Soil_Treatment"), ~ Soil_Treatment, by = "Soil_Type"),
                        method = "pairwise", interaction = TRUE, 
                        adjust = "bonferroni") %>% data.frame() %>% 
      tidyr::separate(Soil_Treatment_pairwise, c("group1", "group2")) %>% 
      dplyr::mutate(across(c(group1, group2), ~trimws(.x)))
    
    pwc_trt <- contrast(emmeans(artlm(m, "Soil_Type"), ~ Soil_Type, by = "Soil_Treatment"),
                        method = "pairwise", interaction = TRUE, 
                        adjust = "bonferroni") %>% data.frame() %>% 
      tidyr::separate(Soil_Type_pairwise, c("group1", "group2")) %>% 
      dplyr::mutate(across(c(group1, group2), ~trimws(.x)))
    
    # Method for adding proper significance labels
    pwc_type_data <- artlm(m, "Soil_Type")$model
    pwc_type_emmeans <- pwc_type_data %>% 
      group_by(Soil_Type) %>% 
      emmeans_test(.y ~ Soil_Treatment, p.adjust.method = "bonferroni") %>% 
      add_xy_position(x = "Soil_Treatment", fun = "mean_sd") %>% 
      dplyr::mutate(y.position = y.position + (0.05 * max(pwc_type_data$.y)))
    
    pwc_trt_data <- artlm(m, "Soil_Treatment")$model
    pwc_trt_emmeans <- pwc_trt_data %>% 
      group_by(Soil_Treatment) %>% 
    ...
    pwc_type_join <- merge(pwc_type, pwc_type_emmeans) %>% 
      add_significance("p.value") %>% 
      dplyr::select(-c(p, p.adj, p.adj.signif, statistic, groups))
    
    pwc_trt_join <- merge(pwc_trt, pwc_trt_emmeans) %>% 
      add_significance("p.value") %>% 
      dplyr::select(-c(p, p.adj, p.adj.signif, statistic, groups))
      # droplevels()
    ...
    # Create box plots
    # bxp_type <- ggboxplot(
    #   df, x = "Soil_Type", y = "soil_type_aligned_ranks", 
    #   color = "Soil_Treatment", palette = "jco") + 
    #   stat_pvalue_manual(pwc_type_join %>% dplyr::filter(p.value <= 0.05), 
    #                      label = "p.value.signif")
    # 
    # bxp_treatment <- ggboxplot(
    #   df, x = "Soil_Treatment", y = "soil_trt_aligned_ranks",
    #   color = "Soil_Type", palette = "jco") +
    #   stat_pvalue_manual(pwc_trt_join %>% dplyr::filter(p.value <= 0.05),
    #                      label = "p.value.signif")
    
    # Create bar plots
    pwc_trt_data_sum <- artlm(m, "Soil_Treatment")$model %>% 
      group_by(Soil_Type, Soil_Treatment) %>% 
      summarise(across(.y, list(mean = ~mean(.x, na.rm = TRUE),
                                sd = ~sd(.x, na.rm = TRUE)), 
                       .names = "{.fn}"), .groups = "drop")
      # droplevels() %>%
      # arrange(Soil_Type, Soil_Treatment)
    
    p <- ggbarplot(
      pwc_trt_data_sum, x = "Soil_Treatment", y = "mean", fill = "Soil_Type",
      color = "black", position = position_dodge(0.8), size = 1,
      width = 0.8, palette = c("#5a5a5a", "#adadad")) + 
      geom_errorbar(
        aes(
          ...
          ymax = mean + sd), position = position_dodge2(NULL, padding = 0.6), 
        width = 0.8, size = 1) +
      stat_pvalue_manual(pwc_trt_join %>% dplyr::filter(p.value <= 0.05), size = 6, 
                         bracket.size = 1, label = "p.value.signif") +
      scale_x_discrete(labels = c(AC = "AC", Ash = "Ash", Con = "Control")) +
      ggtitle(ifelse(i == "RF", "Rough Fescue", "Spotted Knapweed")) +
      xlab("Soil Treatment") +
      ylab(ifelse(dv == "germ_rat", expression(bold(sqrt("Germination Rate"))), 
